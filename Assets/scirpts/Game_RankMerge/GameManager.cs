...
public class GameManager : MonoBehaviour
{
    ...
    public GridCell FindClosestCell(Vector3 popsition)
    {
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                if (grid[x, y].ContainsPosition(popsition))
                {
                    return grid[x, y];
                }
            }
        ...
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                float distance = Vector3.Distance(popsition, grid[x, y].transform.position);
    ...
    public void MergeRanks(DraggableRank draggedRank, DraggableRank tsrgetRank)
    {
        if(draggedRank == null || tsrgetRank == null || draggedRank.rankLevel != tsrgetRank.rankLevel)
        {
            if (draggedRank != null) draggedRank.ReturnToOriginalPosition();
            return;
        }
        int newLevel = tsrgetRank.rankLevel + 1;
